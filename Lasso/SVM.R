rm(list=ls())
training<-read.csv("F:/Thesis/DataMing+MachieLeaning/CAD/Lasso/CAD Dataset Train70p.csv")
testing<-read.csv("F:/Thesis/DataMing+MachieLeaning/CAD/Lasso/CAD Dataset Test30p.csv")
sapdata<-read.csv("F:/Thesis/DataMing+MachieLeaning/CAD/Lasso/Sap_SVM_Selected.csv")

seedarray <- c(4303,4321,123,1234,131,689,320,420,987,150,489,143,323,550,740,430,303,456,970,880)
...
for(i in 1:length(seedarray)){
  ...
  system.time(svm_tune <- tune(svm, train.x=training[,1:27], train.y=as.factor(training[,28]), 
  ...
  print(classifier)
  y_pred=predict(classifier,newdata=testing[-28])
  #y_pred<-round(y_pred)
  cm=table(y_pred,testing[,28],dnn=c("Prediction","Actual"))
  ...
  roc_obj<-roc(testing[,28],as.numeric(y_pred))
  rocauc<-auc(roc_obj)
...
write.csv(sapdata,"F:/Thesis/DataMing+MachieLeaning/CAD/Lasso/Sap_SVM_Selected.csv")
