...
sapdata<-read.csv("F:/Thesis/DataMing+MachieLeaning/CAD/ATLEAST3/Sap_SVM_Selected.csv")

seedarray <- c(12345,4303,4321,123,1234,131,689,320,987,150,489,143,323,550,740,430,303,456,970,880)
...
for(i in 1:length(seedarray)){
  ...
  system.time(svm_tune <- tune(svm, train.x=training[,1:19], train.y=as.factor(training[,20]), 
  ...
  print(classifier)
  y_pred=predict(classifier,newdata=testing[-20])
  #y_pred<-round(y_pred)
  cm=table(y_pred,testing[,20],dnn=c("Prediction","Actual"))
  ...
  roc_obj<-roc(testing[,20],as.numeric(y_pred))
  rocauc<-auc(roc_obj)
...
write.csv(sapdata,"F:/Thesis/DataMing+MachieLeaning/CAD/ATLEAST3/Sap_SVM_Selected.csv")
