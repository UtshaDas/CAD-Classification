...
sapdata<-read.csv("F:/Thesis/DataMing+MachieLeaning/CAD/Common5/Sap_DT_Selected.csv")
...
for(cp_value in 1:length(cp_array)){
  for (i in 1:20) {
    for (j in 1:32) {
      ...
      roc_obj<-roc(testing$Cath,as.numeric(testing$pred))
      rocauc<-auc(roc_obj)
...
write.csv(sapdata,"F:/Thesis/DataMing+MachieLeaning/CAD/Common5/Sap_DT_Selected.csv")
print(cp_op)
print(max_dep)
print(min_split)
print(accu)
