library(neuralnet)
library(pROC)
trainDF<-read.csv("F:/Thesis/DataMing+MachieLeaning/CAD/CAD Dataset Train70p.csv")
testDF<-read.csv("F:/Thesis/DataMing+MachieLeaning/CAD/CAD Dataset Test30p.csv")
...
neuralModel<-neuralnet(formula=form, data=trainDF, hidden = c(16,4,2), threshold = 0.001,
...
PredictionsWithClass <- compute(neuralModel,testDF[,1:66]) 
