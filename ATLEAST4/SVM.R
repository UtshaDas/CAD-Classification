...
sapdata<-read.csv("F:/Thesis/DataMing+MachieLeaning/CAD/ATLEAST4/Sap_SVM_Selected.csv")
...
for(i in 1:length(seedarray)){
  ...
  system.time(svm_tune <- tune(svm, train.x=training[,1:14], train.y=as.factor(training[,15]), 
  ...
  print(classifier)
  y_pred=predict(classifier,newdata=testing[-15])
  #y_pred<-round(y_pred)
  cm=table(y_pred,testing[,15],dnn=c("Prediction","Actual"))
  ...
  roc_obj<-roc(testing[,15],as.numeric(y_pred))
  rocauc<-auc(roc_obj)
...
write.csv(sapdata,"F:/Thesis/DataMing+MachieLeaning/CAD/ATLEAST4/Sap_SVM_Selected.csv")
